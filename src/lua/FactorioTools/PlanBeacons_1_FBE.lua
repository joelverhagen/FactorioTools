  ...
  namespace.class("PlanBeaconsFbe", function (namespace)
    ...
    GetBeacons = function (context, effectEntityAreas, possibleBeacons)
      ...
      if context.Options.OverlapBeacons then
        ...
        default = System.new(KnapcodeOilField.CustomCountedBitArray, 2, effectEntityAreas:getCount())
      end
      local coveredEntityAreas = default
      while possibleBeacons:getCount() > 0 do
        local continue
        repeat
          local beacon = possibleBeacons:get(possibleBeacons:getCount() - 1)
          possibleBeacons:RemoveAt(possibleBeacons:getCount() - 1)
    ...
    GetPossibleBeacons = function (context, effectEntityAreas, possibleBeaconAreas, pointToBeaconCount, pointToEntityArea)
      ...
      local possibleBeacons = KnapcodeOilField.TableArray.New1(possibleBeaconAreas:getCount(), class.BeaconCandidate)
      local effectsGiven = System.new(KnapcodeOilField.CustomCountedBitArray, 2, effectEntityAreas:getCount())
      for i = 0, possibleBeaconAreas:getCount() - 1 do
        local continue
        repeat
          ...
          local originalIndex = possibleBeacons:getCount()
          local default
          if context.Options.OverlapBeacons then
            default = nil
          else
    ...
    GetOccupiedPositions = function (context, entityAreas)
      ...
      for i = 0, entityAreas:getCount() - 1 do
    ...
    GetEntityAreas = function (context)
      local areas = KnapcodeOilField.TableArray.New1(context.Grid:getEntityLocations():getCount(), class.Area)
    ...
    GetEffectEntityAreas = function (entityAreas)
      local effectEntityArea = KnapcodeOilField.TableArray.New(class.Area)
      for i = 0, entityAreas:getCount() - 1 do
        local area = entityAreas:get(i)
        if area.Effect then
          area.Index = effectEntityArea:getCount()
    ...
    GetPossibleBeaconAreas = function (context, occupiedPositions)
      ...
      local possibleBeaconAreas = KnapcodeOilField.TableArray.New(ArrayLocation)
    ...
    GetPointToEntityArea = function (context, effectEntityAreas)
      ...
      for i = 0, effectEntityAreas:getCount() - 1 do
